...
define( 'YIT_LAYERSLIDER_DIR', dirname(__FILE__) );    
define( 'YIT_LAYERSLIDER_URL', YIT_THEME_SLIDERS_URL . '/layer-slider' );  

// Activate the plugin if necessary
if(get_option( get_template() . '_layerslider_activated', '0') == '0') {

    // Run activation script
    layerslider_activation_scripts();

    // Save a flag that it is activated, so this won't run again
    update_option( get_template() . '_layerslider_activated', '1');
}   
...
/********************************************************/
	...
	$GLOBALS['lsPluginPath'] = YIT_LAYERSLIDER_URL . '/';
	...
	// Storage notice
	if(get_option('layerslider-slides', get_option('layerslider_slides')) != false) {
...
function layerslider_init($atts) {
	...
	if(!isset($atts['id']) || empty($atts['id'])) {
		return current_user_can( 'administrator' ) ? '[LayerSliderWP] '.__('Invalid shortcode', 'LayerSlider').'. '.__('<b>Only for admin</b>: Please, check the slider is set properly in "Sliders" admin section.', 'LayerSlider') : '';
	}
	...
	// Result check
	if($slider == null) {
		return current_user_can( 'administrator' ) ? '[LayerSliderWP] '.__('Slider not found', 'LayerSlider').'. '.__('<b>Only for admin</b>: Go to layerslider configuration page and click in the "Convert Data" button inside the admin notice or set it properly in "Sliders" admin section..', 'LayerSlider') : '';
	}
	...
	include(dirname(__FILE__).'/slider_markup.php');
...
function layerslider_convert() {
	...
	// Get sliders if any
	$sliders = get_option('layerslider-slides', get_option('layerslider_slides'));
	...
	foreach($sliders as $key => $slider) {
        ...
        // set responsive
        $slider['properties']['responsive']        = 'true';
        $slider['properties']['forceresponsive']   = 'true';
        $slider['properties']['responsiveunder']   = 1170;
        $slider['properties']['sublayercontainer'] = 1170;
	...
	delete_option('layerslider_slides');
