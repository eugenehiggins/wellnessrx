...
class YIT_Css {
    ...
    public function add_by_option( $option, $value ) {
        ...
        if( $option['id'] == 'header-height' && $value != $option['std'] ) {
            ...
            $this->add( $option['style']['selectors'], array( 'background' => "{$value['color']} url('" . preg_replace( '/http(s)?:/', '', $value['image'] ) . "')" ) );
            ...
            $this->add( $option['style']['selectors'], array( $option['style']['properties'] => "url('" . preg_replace( '/http(s)?:/', '', $value ) . "')" ) );
